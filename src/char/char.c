...
void set_char_online(int map_id, int char_id, int account_id)
{
	...
	{
		ShowNotice("set_char_online: Personagem (%d:%d) marcado no map-server %d, porÈm o map-server %d diz possuir (%d:%d) online!\n",
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2711:
			...
			if (RFIFOB(fd,2)) {
				...
				ShowError("As senhas de comunicaÁ„o com o servidor (padr„o: s1/p1) provavelmente est„o incorretas.\n");
				...
				ShowStatus("N„o foi possÌvel conectar-se ao login-server (conex„o #%d).\n", fd);
				loginif_on_ready();
			}
			RFIFOSKIP(fd,3);
		break;
...
int parse_frommap(int fd)
{
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2afa: // Receiving map names list from the map-server
			...
			int x;
			if (j == 0) {
				ShowWarning("Map-server %d N√O possui mapas.\n", id);
			} else {
...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("pedido de conex„o - account_id:%d/login_id1:%d/login_id2:%d\n", account_id, login_id1, login_id2);
...
int char_lan_config_read(const char *lancfgName)
{
	...
	if((fp = fopen(lancfgName, "r")) == NULL) {
		ShowWarning("Arquivo de configuraÁ„o para Suporte a LAN n„o encontrado: %s\n", lancfgName);
		return 1;
	}

	ShowInfo("Lendo arquivo de configuraÁ„o %s...\n", lancfgName);
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(sscanf(line,"%[^:]: %[^:]:%[^:]:%[^\r\n]", w1, w2, w3, w4) != 4) {
			...
			ShowWarning("Erro de sÌntaxe no arquivo de configuraÁ„o %s na linha %d.\n", lancfgName, line_num);
			continue;
		}
...
int char_config_read(const char* cfgName)
{
	...
	if (fp == NULL) {
		ShowError("Arquivo de configuraÁ„o n„o encontrado: %s.\n", cfgName);
		return 1;
	}
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(strcmpi(w1,"timestamp_format") == 0) {
			...
			if (login_ip) {
				...
				ShowStatus("EndereÁo de IP do login-server : %s -> %s\n", w2, ip2str(login_ip, ip_str));
			}
			...
			if (char_ip){
				...
				ShowStatus("EndereÁo de IP do char-server : %s -> %s\n", w2, ip2str(char_ip, ip_str));
			}
			...
			if (!start_point.map)
				ShowError("start_point %s especificado n„o foi encontrado no caceh do map-index.\n", map);
			start_point.x = x;
			start_point.y = y;
...
int do_init(int argc, char **argv)
{
	...
	if ((naddr_ != 0) && (!login_ip || !char_ip))
	{
		...
		if (naddr_ > 1)
			ShowStatus("Foram detectadas m˙ltiplas interfaces..  usando %s como endereÁo IP\n", ip_str);
		else
			ShowStatus("Padronizando %s como endereÁo IP\n", ip_str);
		if (!login_ip) {
	...
	ShowInfo("Final da funÁ„o de inicializaÁ„o do char-server.\n");

	set_defaultparse(parse_char);
	ShowInfo("porta aberta %d.....\n",char_port);
	char_fd = make_listen_bind(bind_ip, char_port);
	ShowStatus("O char-server est· "CL_GREEN"pronto"CL_RESET" (Servidor est· sendo executado na porta %d).\n\n", char_port);
